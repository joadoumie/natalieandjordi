function doPost(e) {
  try {
    ...
    recipients.forEach(function (addr) {
      MailApp.sendEmail({
        to: addr,
        subject: "RSVP: " + (first.group || "New Response"),
        body: body,
      });
    });
...
function sendConfirmationEmail(guests) {
  ...
  guests.forEach(function (g, idx) {
    try {
      ...
      Logger.log(
        "Sending confirmation to guest " + idx + ": " + g.name + " at " + email,
      );
      ...
      lines.push("");
      lines.push(
        "We've received your RSVP for Natalie & Jordi's wedding! Here's a summary:",
      );
      lines.push("");
      ...
      if (onsiteCount > 0) {
        ...
        lines.push(
          "The fee for staying onsite at Wildhaven is $275 per person.",
        );
        lines.push(
      ...
      lines.push(
        "If anything changes, just submit the form again and we'll update your info.",
      );
      lines.push("");
